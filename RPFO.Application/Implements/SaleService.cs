...
namespace RPFO.Application.Implements
{
    public partial class SaleService : ISaleService
    {
        ...
        private readonly IGenericRepository<TTableGroup> _tableGroupRepository;
        private readonly IGenericRepository<TPlacePrint> _placePrintGroupRepository;
        ...
        private readonly ITerminalPeripherallService _peripherallService;
        private readonly IThirdPartyLogService _log3Service;
        private readonly IPaymentMethodService _paymentService;
        private readonly IFormatConfigService _formatConfigService;
        ...
        private readonly string ComBankTerminal = "";
        private IShiftService _shiftService;
        private ICurrencyRoundingOffService _currencyRoundingOffService;
        ...
        //
        IMapper _mapper;
        string ServiceNameLog = "S_Log";

        string MWIAuthUser = "user_test";
        string MWIAuthPassword = "1234";
        string MWIAuthCustom = "";

        List<string> TableNameList = new List<string>();
        public SaleService(IGenericRepository<TSalesHeader> saleHeaderRepository, IBOMService bomeService, ITerminalPeripherallService peripherallService,
            ICommonService commonService, IPermissionService permissionService, IFormatConfigService formatConfigService, IGenericRepository<TTableGroup> tableGroupRepository,
            ...
            IGenericRepository<TSalesPayment> salepaymentLineRepository, IConfiguration config, IInvoiceService invoiceService, IItemService itemService,
           ...
           IGeneralSettingService settingService, IPrepaidCardService prepaidCardService, IThirdPartyLogService log3Service,
           IGenericRepository<TPlacePrint> placePrintGroupRepository, IGenericRepository<MTablePlace> tablePlaceRepository, IGenericRepository<MItem> itemRepository,
           ILoyaltyService loyaltyService, IGenericRepository<MCustomer> customerRepository, IMapper mapper, ICurrencyRoundingOffService currencyRoundingOffService,
           IShiftService shiftService, IResponseCacheService responseCacheService /*, IHubContext<RequestHub> hubContext*/
         )//: base(hubContext)
        {
            ...
            _peripherallService = peripherallService;
            _itemService = itemService;
            _currencyRoundingOffService = currencyRoundingOffService;
            ...
            _formatConfigService = formatConfigService;
            _tableGroupRepository = tableGroupRepository;
            _placePrintGroupRepository = placePrintGroupRepository;
            ...
            ComBankTerminal = Utilities.Helpers.Encryptor.DecryptString(config.GetConnectionString("ComBankTerminal"), RPFO.Utilities.Constants.AppConstants.TEXT_PHRASE);
            ...
            string AuthUser = Utilities.Helpers.Encryptor.DecryptString(config.GetConnectionString("MWIAuthUser"), RPFO.Utilities.Constants.AppConstants.TEXT_PHRASE);
            if (!string.IsNullOrEmpty(AuthUser))
            {
                MWIAuthUser = AuthUser;
            }
            string AuthPassword = Utilities.Helpers.Encryptor.DecryptString(config.GetConnectionString("MWIAuthPassword"), RPFO.Utilities.Constants.AppConstants.TEXT_PHRASE);
            if (!string.IsNullOrEmpty(AuthPassword))
            {
                MWIAuthPassword = AuthPassword;
            }
            string AuthCustom = Utilities.Helpers.Encryptor.DecryptString(config.GetConnectionString("MWIAuthCustom"), RPFO.Utilities.Constants.AppConstants.TEXT_PHRASE);
            if (!string.IsNullOrEmpty(AuthCustom))
            {
                MWIAuthCustom = AuthCustom;
            }
            TableNameList.Add("S_Log");
            _commonService.InitService(ServiceNameLog, TableNameList);
        ...
        public GenericResult initService()
        {
            ...
            using (IDbConnection db = _saleHeaderRepository.GetConnection())
            {
                try
                {
                    ...
                    string queryCheckAndCreate = "IF (OBJECT_ID('USP_S_CounterInforEpay') IS NULL)  begin declare @string nvarchar(MAX) = '';" +
                        " set @string = 'create PROCEDURE [dbo].[USP_S_CounterInforEpay] @CompanyCode nvarchar(50), @StoreId	nvarchar(150), @TerminalId	nvarchar(150) AS " +
                        "begin  select * from S_StoreClient  " +
                        "where CompanyCode = @CompanyCode and StoreId = @StoreId and PublicIP = @TerminalId end '; " +
                        "EXECUTE sp_executesql @string;  end";
                    db.Execute(queryCheckAndCreate);

                    string queryCheckAndCreateSlog = "IF (OBJECT_ID('USP_I_S_LogByTableType') IS NULL)  begin declare @string nvarchar(MAX) = '';" +
                     " set @string = 'create PROCEDURE [dbo].[USP_I_S_LogByTableType] @CompanyCode nvarchar(50), @StoreId	nvarchar(150), @TransId	nvarchar(150), @TerminalId	nvarchar(150), 	@CreatedBy	nvarchar(150)= null,  @Lines as S_LogTableType READONLY    AS " +
                     ...
                     " end end'; " +
                     "EXECUTE sp_executesql @string;  end";
                    db.Execute(queryCheckAndCreateSlog);
        ...
        public GenericResult checkInitService(string CompanyCode, string StoreId, string SettingId)
        {
            GenericResult result = new GenericResult();
            ...
            using (IDbConnection db = _saleHeaderRepository.GetConnection())
            {
                try
                {
                    ...
                    queryCheckAndCreate = $"if (select count (*) from dbo.fn_GetGeneralSettingStore (N'{CompanyCode}', N'{StoreId}', N'{SettingId}')) <> 0 begin " +
                        "IF (OBJECT_ID('USP_Calculate_StaffCommission') IS NULL)  begin declare @string nvarchar(MAX) = '';" +
                      " set @string = 'create PROCEDURE [dbo].[[USP_Calculate_StaffCommission] @CompanyCode nvarchar(50), @StoreId	nvarchar(150), @TransId	nvarchar(150) AS " +
                      " begin  update T_SalesStaff set  [Percent] = t2.[Percent]  , [Amount] = (isnull(t2.[Percent],0) * t3.TotalPayable) / 100   " +
                      " from T_SalesStaff  t1 with (nolock) left join S_SalesPlan t2 with (nolock) on t1.CompanyCode = t2.CompanyCode and t2.Value = t1.Position left join T_SalesHeader t3 with (nolock) on t1.CompanyCode = t2.CompanyCode and t1.TransId = t3.TransId " +
                      " where t1.CompanyCode = @CompanyCode and t1.TransId = @TransId and t3.StoreId = @StoreId end '; " +
                      " EXECUTE sp_executesql @string;  end" +
                      " end";
                    db.Execute(queryCheckAndCreate);
        ...
        public Document MapSOtoDocument(SaleViewModel model)
        {
            ...
            foreach (var line in model.Lines)
            {
                ...
                lineDo.SerialNum = line.SerialNum;
        ...
        //public GenericResult CreateFromAnotherSO(SaleViewModel model)
        //{
        //    GenericResult result = new GenericResult();
        //    if(string.IsNullOrEmpty(model.BillList))
        //    {
        //        result.Success = false;
        //        result.Message = "From Another Bill Can't null. Please check";
        //        return result;
        //    }
        //    if (string.IsNullOrEmpty(model.StoreId))
        //    {
        //        result.Success = false;
        //        result.Message = "StoreId Can't null. Please check";
        //        return result;
        //    }
        //    if (string.IsNullOrEmpty(model.ContractNo))
        //    {
        //        result.Success = false;
        //        result.Message = "TableId Can't null. Please check";
        //        return result;
        //    }
        //    if (string.IsNullOrEmpty(model.CreatedBy))
        //    {
        //        result.Success = false;
        //        result.Message = " Created By From Another Bill Can't null. Please check";
        //        return result;
        //    }
        //    if (string.IsNullOrEmpty(model.DataSource))
        //    {
        //        model.DataSource = "POS";
        //    }

        //    try
        //    { 
        //        List<string> billInSt = model.BillList.Split(",").ToList();


        //        model.Lines = new List<TSalesLineViewModel>();




        //        if (billInSt!=null && billInSt.Count() > 0)
        //        {
        //            foreach(string billno in billInSt)
        //            {
        //               var bill = GetOrderById(billno, model.CompanyCode, model.StoreId).Result;
        //               if(bill.Success)
        //               {
        //                    var billData = bill.Data as SaleViewModel;
        //                    model.Lines.AddRange(billData.Lines); 
        //                    model.PromoLines.AddRange(billData.PromoLines);
        //                    model.SerialLines.AddRange(billData.PromoLines);
        //                }   
        //               else
        //               {
        //                    result.Success = false;
        //                    result.Message = "Get data of bill " + billno + " failed.";
        //                    return result;
        //               }    
        ...
        //        {
        //            result.Success = false;
        //            result.Message = "Bill list can't null";
        //            return result;
        //        }    
        //    }   
        //    catch(Exception ex)
                    ...
                    try
                    {
                        ...
                        using (var tran = db.BeginTransaction())
                        {
                            ...
                            try
                            {
                                ...
                                var checkCompValue = _saleHeaderRepository.GetScalar(CheckCompStr, null, commandType: CommandType.Text);
                                ...
                                if (model.IsCanceled == "C" && (model.Status.ToLower() == "h" || model.Status.ToLower() == "hold"))
                                {
                                    ...
                                    string statusField = _saleHeaderRepository.GetScalar(checkOrderStatus, null, commandType: CommandType.Text);
                                            ...
                                            if (MWIEpay != null)
                                            {
                                                ...
                                                if (model.SalesMode == "SALES" && (line.ItemType.ToLower() == "pn" || line.ItemType.ToLower() == "pin"))
                                                {
                                                    ...
                                                    for (int iP = 1; iP <= line.Quantity; iP++)
                                                    {
                                                        ...
                                                        var resultEpay = await EpayPINOrder(MWIEpay.CustomField1, MWIEpay.CustomField3, MWIEpay.CustomField2, line.Price * 100, model.StoreId, line.ProductId, model.OrderId.ToString() + "-" + line.LineId + "-" + iP);
                                                        if (resultEpay != null && resultEpay.Success)
                                                        {
                                                            ...
                                                            newSerialLine.ExpDate = null;
                                                            if (!string.IsNullOrEmpty(data.pinExpiryDate))
                                                            {

                                                                var date = DateTime.Now.ToString("yyyy").Substring(0, 2) + data.pinExpiryDate.Substring(0, 2) + "/" + data.pinExpiryDate.Substring(2, 2) + "/" + data.pinExpiryDate.Substring(4, 2);
                                                                newSerialLine.ExpDate = DateTime.Parse(date);
                                                            }

                                                            newSerialLine.Prefix = "PIN";
                                                            newSerialLine.CustomF2 = line.ProductId;
                                                            newSerialLine.CustomF3 = ToHtml(data.customField1);
                                                            model.SerialLines.Add(newSerialLine);

                                                            EpayModel epay = new EpayModel();
                                                            epay.terminalID = MWIEpay.CustomField3;
                                                            epay.merchantID = MWIEpay.CustomField2;
                                                        ...
                                                        {
                                                            lineLog.EndTime = DateTime.Now;
                                                            string message = $"Epay Message: URL(" + MWIEpay.CustomField1 + "), Terminal: " + MWIEpay.CustomField3 + ", Merchand: " + MWIEpay.CustomField2 + " Msg: " + resultEpay.Message;
                                                            lineLog.Remark = message;
                                                            lineLog.Status = "False";
                                                            lineLog.CustomF5 = resultEpay.ToJson();
                                                            log.Lines.Add(lineLog);
                                                            tran.Rollback();
                                                            result.Success = false;
                                                            result.Code = 2000;
                                                            ...
                                                            var resultVoid = await VoidEpay(MWIEpay.CustomField1, epayList, model.OrderId.ToString() + "-" + line.LineId);
                                                ...
                                                if (line.ItemType.ToLower() == "tp" || line.ItemType.ToLower() == "topup" || line.ItemType.ToLower() == "bp")
                                                {
                                                    ...
                                                    var resultEpay = await EpayTopupOrder(MWIEpay.CustomField1, MWIEpay.CustomField3, MWIEpay.CustomField2, line.LineTotal * 100, model.StoreId, line.ProductId, line.Custom1, model.OrderId.ToString() + "-" + line.LineId);
                                                    ...
                                                    if (resultEpay != null && resultEpay.success.HasValue && resultEpay.success.Value)
                                                    {
                                                        var data = JsonConvert.DeserializeObject<EpayResult>(resultEpay.Data.ToString());
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        result.Success = false;
                                                        result.Code = 2000;
                                                        string message = $"Epay Message: URL(" + MWIEpay.CustomField1 + "), Terminal: " + MWIEpay.CustomField3 + ", Merchand: " + MWIEpay.CustomField2 + " Msg: " + resultEpay.Msg;
                                                        result.Message = message;
                                                        lineLog.EndTime = DateTime.Now;
                                                        //lineLog.Remark = "Epay Message: " + resultEpay.Msg;
                                                        lineLog.Remark = message;
                                                        lineLog.Status = "False";
                                                        lineLog.CustomF5 = resultEpay.ToJson();
                                                        log.Lines.Add(lineLog);
                                    ...
                                    foreach (var payment in model.Payments)
                                    {
                                        ...
                                        if (model.IsCanceled == "C" && model.Status != "H" && !string.IsNullOrEmpty(payment.PaymentMode) && payment.PaymentMode == "BankTerminal")
                                        {
                                            if (payment.PaymentMode == "BankTerminal")
                                            {
                                                ...
                                                //( select FatherId from M_PaymentMethod with(nolock) where PaymentCode = N'{payment.PaymentCode}' and CompanyCode = N'{model.CompanyCode}')";
                                                string GetPort = _saleHeaderRepository.GetScalar(queryGetPort, null, commandType: CommandType.Text);
                                                    ...
                                                    if (resultHold != null && resultHold.success.Value)
                                                    {
                                                        ...
                                                        if (data != null)
                                                        {
                                                            if (data.statuscode != voucherRedeem.statuscode)
                                                            {
                                                                tran.Rollback();
                                                                result.Success = false;
                                                                result.Message = "S4 Voucher Message: " + data.statusmessage;
                                                                return result;
                                                            }
                                ...
                                if (model.Status.ToLower() != "h" && model.Status.ToLower() != "hold")
                                {
                                    ...
                                    {
                                        var luckyModel = _loyaltyService.GetLuckyNo(document, out string _);

                                        if (luckyModel != null && !string.IsNullOrEmpty(luckyModel.LuckyNo))
                                        {
                                            db.Execute($"Update T_SalesHeader set LuckyNo = '{luckyModel.LuckyNo}' where TransId = N'{model.TransId}' and CompanyCode = N'{model.CompanyCode}' and StoreId= N'{model.StoreId}'", null, commandType: CommandType.Text, transaction: tran);
                                        }
                                    }
                                }
                                ...
                                {
                                    await WriteFileLog(model, false);
                                }
        ...
        public async Task<GenericResult> CreateSaleOrderByTableType(SaleViewModel model)
        {
            ...
            if (model != null && model.SalesType == "Table")
            {
                RPFO.Utilities.Helpers.LogUtils.WriteLogData("C:\\RPFO.API.Log\\", "SalesOrderInTable", "CreateSOInTable", model.ToJson());
            }
            else
            {
                RPFO.Utilities.Helpers.LogUtils.WriteLogData("C:\\RPFO.API.Log\\", "SalesOrders", "CreateSO", model.ToJson());
            ...
            checkInitService(model.CompanyCode, model.StoreId, "AssignStaff");
            try
            {
                #region check data
                if (string.IsNullOrEmpty(model.DataSource))
                ...
                int DecimalPlacesFormat = 6;
                if (model.Payments != null && model.Payments.Count > 0)
                {

                    var configOfStoreGet = await _formatConfigService.GetByStore(model.CompanyCode, model.StoreId);
                    if (configOfStoreGet != null)
                    {
                        if (configOfStoreGet.Success == true && configOfStoreGet.Data != null)
                        {
                            var format = configOfStoreGet.Data as SFormatConfig;
                            if (format != null)
                            {
                                if (!string.IsNullOrEmpty(format.DecimalPlacesFormat))
                                {
                                    DecimalPlacesFormat = int.Parse(format.DecimalPlacesFormat);
                                }
                            }
                        }
                    ...
                    //.GetByStore(model.CompanyCode, model.StoreId);
                    // Kiểm tra xem Total receip và Total Collected có bằng nhau không
                    if (model.DataSource.ToLower() == "pos" && model.Status == "C")
                    {
                        var CalcuColected = model.Payments.Sum(x => (x.CollectedAmount));
                        CalcuColected = Math.Round((CalcuColected.HasValue ? CalcuColected.Value : 0), DecimalPlacesFormat);
                        //+x.PaymentDiscount ?? 0

                        // + model.PaymentDiscount??0
                        if (CalcuColected != (model.TotalReceipt))
                        {
                            await WriteFileLog(model, false);
                            result.Success = false;
                            result.Message = $"Total amount collected ({CalcuColected}) and Total receipt ({model.TotalReceipt}) do not match";
                    ...
                    var payoo = model.Payments.Where(x => x.PaymentCode == "PAYOO" && x.CustomF2 == "E" && model.DataSource.ToLower() == "pos");
                    if (payoo != null && payoo.Count() > 0)
                    {
                        if (model.Status != "I" && model.IsCanceled.ToLower() != "c")
                        {
                            model.Status = "I";//set status lại cho đơn payoo;
                            model.CollectedStatus = "InProcess";
                        }
                        else
                        {
                            model.Status = "C";
                            model.CollectedStatus = "Completed";
                        }
                ...
                var settingPayoo = SettingList.FirstOrDefault().GeneralSettings.Where(x => x.SettingId == "Payoo").FirstOrDefault();

                if (model.Status == "I")
                {
                    if (settingPayoo == null || string.IsNullOrEmpty(settingPayoo.CustomField1))
                    {
                        result.Success = false;
                        result.Message = "Cannot found Setting of Payoo!";
                        result.Data = model;
                        return result;
                    }
                }
                //  Minh đã xin phép Lập    (2021-11-23)
                if ((string.IsNullOrEmpty(model.TransId) && model.DataSource == "GRAB") || (model.DataSource.ToLower() == "ecom" && model.Status.ToLower() == "h"))
                {
                    //model.Status = "C";
                    //model.CollectedStatus = "Closed";
                    var Ecomparameters = new DynamicParameters();
                ...
                var SalesMinQty = SettingList.FirstOrDefault().GeneralSettings.Where(x => x.SettingId == "SalesMinQty").FirstOrDefault();
                if (SalesMinQty != null && !string.IsNullOrEmpty(SalesMinQty.SettingValue) && SalesMinQty.SettingValue != "0")
                {
                    decimal? minValue = Math.Abs(decimal.Parse(SalesMinQty.SettingValue));
                    var checkItem = model.Lines.Where(x => Math.Abs(x.Quantity.Value) < minValue).ToList();
                    if (checkItem != null && checkItem.Count > 0)
                    {
                        result.Success = false;
                        result.Message = "Can't completed order. Quantity of item " + checkItem[0].ItemName + " invalid";
                        return result;
                    }
                }
                ...
                if (model.DataSource.ToLower() == "pos" && model.Status.ToLower() != "h" && model.Status.ToLower() != "hold")
                {
                    ...
                    {
                        result.Success = false;
                        result.Message = "Shift " + model.ShiftId + " does not exist or has closed.";
                        return result;
                    }
                    else
                    {
                        var created = shift.CreatedOn.Value.ToString("yyyy/MM/dd");
                        var now = DateTime.Now.ToString("yyyy/MM/dd");
                        var shiftId = shift.DailyId;
                        var dailyIdQury = $"select dbo.[fnc_GetDailyID]( N'{model.StoreId}', N'{model.CompanyCode}', N'{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}') ";
                        var nowDailyId = _saleHeaderRepository.GetScalar(dailyIdQury, null, commandType: CommandType.Text);

                        if (nowDailyId != shiftId)
                        {
                            result.Success = false;
                            result.Message = "Can't completed order. Shift " + model.ShiftId + " not valid. Please logout";
                            return result;
                    ...
                    var lineDisNull = model.Lines.Where(x => string.IsNullOrEmpty(x.BomId)).Where(x => string.IsNullOrEmpty(x.DiscountType)).ToList();

                    if (lineDisNull != null && lineDisNull.Count() > 0)
                    {
                        var itemX = lineDisNull.FirstOrDefault();
                        result.Success = false;
                        result.Message = "ItemCode:" + itemX.ItemCode + " ItemName: " + itemX.ItemName + " discount type is null";
                        return result;
                    }
                    var salesZeroValue = SettingList.FirstOrDefault().GeneralSettings.Where(x => x.SettingId == "SalesZeroValue").FirstOrDefault();
                    if (salesZeroValue != null && (salesZeroValue.SettingValue == "false" || salesZeroValue.SettingValue == "0"))
                    ...
                    if (!string.IsNullOrEmpty(model.IsCanceled) && model.IsCanceled == "N" && model.SalesType.ToLower() != "table")
                    {
                        decimal TotalDiscount = model.TotalDiscountAmt ?? 0;
                        decimal DiscountInLine = 0;
                        foreach (var line in model.Lines.Where(x => (string.IsNullOrEmpty(x.BomId) ? "" : x.BomId) == "" && x.ItemType.ToLower() != "card"))
                        {
                            DiscountInLine += (line.DiscountAmt.HasValue ? line.DiscountAmt.Value : 0);
                            // ((line.LineTotalBefDis.HasValue ? line.LineTotalBefDis.Value : (line.Quantity.Value * line.Price.Value)) - (line.LineTotalDisIncludeHeader.HasValue ? line.LineTotalDisIncludeHeader.Value : 0));
                        }
                        DiscountInLine += (model.DiscountAmount.HasValue ? model.DiscountAmount.Value : 0);
                        var percen1 = (DiscountInLine * 1 / 100);
                        var Minus1 = DiscountInLine - percen1;
                        var Bonus1 = DiscountInLine + percen1;
                        if (Math.Abs(TotalDiscount) < Math.Abs(Minus1) || Math.Abs(TotalDiscount) > Math.Abs(Bonus1))
                        {
                            result.Success = false;
                            result.Message = "Total discount invalid. Please check discount in bill or ReOrder. Thank you.";
                            return result;
                ...
                //if (model.POSType != "E" && (model.DataSource.ToLower() == "pos" && model.Status != "H") || (model.DataSource.ToLower() != "pos" && model.POSType != "E"))
                if (model.DataSource.ToLower() == "pos" && string.IsNullOrEmpty(model.ContractNo) && model.Status.ToLower() != "h" && model.Status.ToLower() != "hold" && model.POSType != "m")
                {
                    if (model.TotalPayable != 0)
                    {
                        if (model.Payments == null || model.Payments.Count <= 0)
                ...
                //if (model.DataSource.ToLower() == "pos" )
                //{
                //    if(model.TotalDiscountAmt.HasValue && model.TotalDiscountAmt.Value > 0)
                //    {
                //        var Discount = !model.DiscountAmount.HasValue ? 0 : model.DiscountAmount.Value;
                //        model.Lines.Where
                        ...
                        if (itemListFilter != null && itemListFilter.Success && itemListFilter.Data != null)
                        {
                            ...
                            if (itemList != null && itemList.Count > 0)
                            {
                                if (itemList.Count < itemCheckList.Count)
                                {
                                    ...
                                    result.Success = false;
                                    result.Message = "Please check Items: " + ItemFailed + " in store " + model.StoreId;
                                    return result;
                                }
                            }
                            else
                ...
                if (model.DataSource.ToLower() == "pos" && model.IsCanceled != "C" && model.Payments != null && model.Payments.Count > 0 && model.SalesType.ToLower() != "table")
                {
                    decimal? numOfPayment = 0;
                    decimal? numOfLine = 0;
                    foreach (var line in model.Payments)
                    ...
                    foreach (var line in model.Payments.Where(x => x.PaymentMode.ToLower() != "change"))
                    {
                        ...
                        decimal discount = line.PaymentDiscount.HasValue ? line.PaymentDiscount.Value : (decimal)0;
                        numOfPayment += (Math.Abs(line.CollectedAmount.Value) + Math.Abs(discount));// * (line.Rate ?? 1);
                    }
                    var lines = model.Lines.ToDataTable();
                    ...
                    decimal discountCheck = Math.Round(discountLine + Math.Abs(model.DiscountAmount ?? 0), DecimalPlacesFormat);
                    //XXXXY
                    string currencyOff = _saleHeaderRepository.GetScalar($"select dbo.[fnc_GetRoundingPaymentDifByDefCurStore](N'{model.CompanyCode}','{model.StoreId}')", null, commandType: CommandType.Text);
                    decimal paymentDif = decimal.Parse(currencyOff);
                    if ((Math.Round(Math.Abs(discountTotal), DecimalPlacesFormat) + paymentDif) < discountCheck)
                    {
                        result.Success = false;
                        result.Message = "502: Please check bill and amount. Discount Total: " + discountTotal + ", Discount Line " + discountCheck;
                        return result;
                ...
                if (model.DataSource.ToLower() == "pos" && model.IsCanceled != "C" && string.IsNullOrEmpty(model.ContractNo) && model.SalesType.ToLower() != "table")
                {
                    decimal roudingoff = model.RoundingOff ?? 0;
                    var TotalAmount = Math.Abs((model.TotalPayable ?? 0) - roudingoff);
                    var TotalDiscount = Math.Abs(model.TotalDiscountAmt.Value);
                    var TotalPayable = Math.Abs(model.TotalPayable.Value);
                    decimal perTotalPayable = Math.Abs(model.TotalPayable.Value) * 5 / 100;
                    ...
                    if (TotalAmount - TotalDiscount > (TotalPayable + perTotalPayable) + paymentDif && model.SalesMode.ToLower() == "sales" && model.Status != "H")
                    {
                        result.Success = false;
                        result.Message = "101: Please check your receipt amount. Total Amount: " + model.TotalAmount.Value.ToString("C2") + " Total Discount Amt: " + model.TotalDiscountAmt.Value.ToString("C2") + " TotalPayable: " + model.TotalPayable.Value.ToString("C2") + " TotalReceipt " + model.TotalReceipt.Value.ToString("C2");
                        return result;
                    }
                    if (Math.Abs(model.TotalPayable.Value) > (Math.Abs(model.TotalReceipt.Value) + Math.Abs(model.PaymentDiscount.Value) + paymentDif) && model.SalesMode.ToLower() == "sales" && model.Status != "H")
                    {
                        result.Success = false;
                        result.Message = "102: Please check your receipt amount. Total Payable: " + model.TotalPayable.Value.ToString("C2") + " Total Receipt: " + model.TotalReceipt.Value.ToString("C2") + " TotalPayable: " + model.TotalPayable.Value.ToString("C2") + " TotalReceipt " + model.TotalReceipt.Value.ToString("C2");
                        return result;
                    }
                    if ((TotalAmount - TotalDiscount) > (TotalPayable + paymentDif) && model.SalesMode.ToLower() != "sales" && model.Status == "H")
                    {
                        result.Success = false;
                        result.Message = "103: Please check your receipt amount. Total Amount: " + model.TotalAmount.Value.ToString("C2") + " Total Discount Amt: " + model.TotalDiscountAmt.Value.ToString("C2") + "  TotalPayable: " + model.TotalPayable.Value.ToString("C2") + " TotalReceipt " + model.TotalReceipt.Value.ToString("C2");
                        return result;
                ...
                using (IDbConnection db = _saleHeaderRepository.GetConnection())
                {
                    try
                    {
                        ...
                        using (var tran = db.BeginTransaction())
                        {
                            ...
                            var SOLines = new DataTable("SalesLineTableType");
                            ...
                            var SODeliverry = new DataTable("Sales_DeliveryTableType");
                            SODeliverry.Columns.Add("Id", typeof(string));
                            SODeliverry.Columns.Add("TransId", typeof(string));
                            SODeliverry.Columns.Add("CompanyCode", typeof(string));
                            SODeliverry.Columns.Add("DeliveryType", typeof(string));
                            SODeliverry.Columns.Add("DeliveryMethod", typeof(string));
                            SODeliverry.Columns.Add("DeliveryFee", typeof(decimal));


                            var SOStaffTableType = new DataTable("T_SalesStaffTableType");
                            SOStaffTableType.Columns.Add("CompanyCode", typeof(string));
                            SOStaffTableType.Columns.Add("StoreId", typeof(string));
                            SOStaffTableType.Columns.Add("TransId", typeof(string));
                            SOStaffTableType.Columns.Add("ItemLine", typeof(string));
                            SOStaffTableType.Columns.Add("LineId", typeof(int));
                            SOStaffTableType.Columns.Add("Staff", typeof(string));
                            SOStaffTableType.Columns.Add("Position", typeof(string));
                            SOStaffTableType.Columns.Add("Percent", typeof(decimal));
                            SOStaffTableType.Columns.Add("Amount", typeof(decimal));
                            SOStaffTableType.Columns.Add("Remark", typeof(string));
                            SOStaffTableType.Columns.Add("CreatedBy", typeof(string));
                            SOStaffTableType.Columns.Add("CreatedOn", typeof(DateTime));
                            SOStaffTableType.Columns.Add("ModifiedBy", typeof(string));
                            SOStaffTableType.Columns.Add("ModifiedOn", typeof(DateTime));
                            SOStaffTableType.Columns.Add("Status", typeof(string));
                            SOStaffTableType.Columns.Add("CustomF1", typeof(string));
                            SOStaffTableType.Columns.Add("CustomF2", typeof(string));
                            SOStaffTableType.Columns.Add("CustomF3", typeof(string));
                            SOStaffTableType.Columns.Add("CustomF4", typeof(string));
                            SOStaffTableType.Columns.Add("CustomF5", typeof(string));


                            var SOPromo = new DataTable("SalesPromoTableType");
                            SOPromo.Columns.Add("Id", typeof(string));
                            SOPromo.Columns.Add("TransId", typeof(string));
                            SOPromo.Columns.Add("CompanyCode", typeof(string));
                            SOPromo.Columns.Add("ItemCode", typeof(string));
                            SOPromo.Columns.Add("BarCode", typeof(string));
                            SOPromo.Columns.Add("RefTransId", typeof(string));
                            SOPromo.Columns.Add("ApplyType", typeof(string));
                            SOPromo.Columns.Add("ItemGroupId", typeof(string));
                            SOPromo.Columns.Add("UOMCode", typeof(string));
                            SOPromo.Columns.Add("Value", typeof(decimal));
                            SOPromo.Columns.Add("PromoId", typeof(string));
                            SOPromo.Columns.Add("PromoType", typeof(string));
                            SOPromo.Columns.Add("PromoTypeLine", typeof(string));
                            SOPromo.Columns.Add("CreatedBy", typeof(string));
                            SOPromo.Columns.Add("CreatedOn", typeof(DateTime));
                            SOPromo.Columns.Add("ModifiedBy", typeof(string));
                            SOPromo.Columns.Add("ModifiedOn", typeof(DateTime));
                            SOPromo.Columns.Add("Status", typeof(string));
                            SOPromo.Columns.Add("PromoPercent", typeof(decimal));
                            SOPromo.Columns.Add("PromoAmt", typeof(decimal));
                            SOPromo.Columns.Add("StoreId", typeof(string));



                            var SOInvoice = new DataTable("SalesInvoiceTableType");
                            SOInvoice.Columns.Add("TransId", typeof(string));
                            SOInvoice.Columns.Add("CompanyCode", typeof(string));
                            SOInvoice.Columns.Add("StoreId", typeof(string));
                            SOInvoice.Columns.Add("StoreName", typeof(string));
                            SOInvoice.Columns.Add("CustomerName", typeof(string));
                            SOInvoice.Columns.Add("TaxCode", typeof(string));
                            SOInvoice.Columns.Add("Email", typeof(string));
                            SOInvoice.Columns.Add("Address", typeof(string));
                            SOInvoice.Columns.Add("Phone", typeof(string));
                            SOInvoice.Columns.Add("Remark", typeof(string));
                            SOInvoice.Columns.Add("CreatedOn", typeof(DateTime));
                            SOInvoice.Columns.Add("CreatedBy", typeof(string));
                            SOInvoice.Columns.Add("ModifiedOn", typeof(DateTime));
                            SOInvoice.Columns.Add("ModifiedBy", typeof(string));
                            SOInvoice.Columns.Add("Name", typeof(string));



                            var SOLineSerials = new DataTable("SalesLineSerialTableType");
                            ...
                            //@LinePayments
                            var SOPayments = new DataTable("SalesPaymentTableType");
                            SOPayments.Columns.Add("PaymentCode", typeof(string));
                            SOPayments.Columns.Add("CompanyCode", typeof(string));
                            SOPayments.Columns.Add("TransId", typeof(string));
                            SOPayments.Columns.Add("LineId", typeof(string));
                            SOPayments.Columns.Add("TotalAmt", typeof(decimal));
                            SOPayments.Columns.Add("ReceivedAmt", typeof(decimal));
                            SOPayments.Columns.Add("PaidAmt", typeof(decimal));
                            SOPayments.Columns.Add("ChangeAmt", typeof(decimal));
                            SOPayments.Columns.Add("PaymentMode", typeof(string));
                            SOPayments.Columns.Add("CardType", typeof(string));
                            SOPayments.Columns.Add("CardHolderName", typeof(string));
                            SOPayments.Columns.Add("CardNo", typeof(string));
                            SOPayments.Columns.Add("VoucherBarCode", typeof(string));
                            SOPayments.Columns.Add("VoucherSerial", typeof(string));
                            SOPayments.Columns.Add("CreatedBy", typeof(string));
                            SOPayments.Columns.Add("CreatedOn", typeof(DateTime));
                            SOPayments.Columns.Add("ModifiedBy", typeof(string));
                            SOPayments.Columns.Add("ModifiedOn", typeof(DateTime));
                            SOPayments.Columns.Add("Status", typeof(string));
                            SOPayments.Columns.Add("ChargableAmount", typeof(decimal));
                            SOPayments.Columns.Add("PaymentDiscount", typeof(decimal));
                            SOPayments.Columns.Add("CollectedAmount", typeof(decimal));
                            SOPayments.Columns.Add("RefNumber", typeof(string));
                            SOPayments.Columns.Add("DataSource", typeof(string));
                            SOPayments.Columns.Add("Currency", typeof(string));
                            SOPayments.Columns.Add("Rate", typeof(decimal));
                            SOPayments.Columns.Add("FCAmount", typeof(decimal));
                            SOPayments.Columns.Add("ShiftId", typeof(string));
                            SOPayments.Columns.Add("CardExpiryDate", typeof(DateTime));
                            SOPayments.Columns.Add("AdjudicationCode", typeof(string));
                            SOPayments.Columns.Add("AuthorizationDateTime", typeof(DateTime));
                            SOPayments.Columns.Add("TerminalId", typeof(string));
                            SOPayments.Columns.Add("RoundingOff", typeof(decimal));
                            SOPayments.Columns.Add("FCRoundingOff", typeof(decimal));
                            SOPayments.Columns.Add("ForfeitCode", typeof(string));
                            SOPayments.Columns.Add("Forfeit", typeof(string));
                            SOPayments.Columns.Add("CustomF1", typeof(string));
                            SOPayments.Columns.Add("CustomF2", typeof(string));
                            SOPayments.Columns.Add("CustomF3", typeof(string));
                            SOPayments.Columns.Add("CustomF4", typeof(string));
                            SOPayments.Columns.Add("CustomF5", typeof(string));
                            SOPayments.Columns.Add("StoreId", typeof(string));
                            ...
                            try
                            {
                                ...
                                if (excludeBOMLines != null && excludeBOMLines.Any())
                                {
                                        ...
                                        if (bom.Success)
                                        {
                                            ...
                                            if (bomModel != null && bomModel.Lines != null && bomModel.Lines.Count > 0)
                                            {
                                                ...
                                                if (chekItemBOMLine == null || chekItemBOMLine.Count() == 0)
                                                {
                                                    ...
                                                    {
                                                        var coppyLine = new TSalesLineViewModel();
                                ...
                                //Khi làm đơn hàng cho bàn thì Prefix + thêm ký tự T để làm đơn tạm...
                                //Khi Thanh toán thì toàn bộ những đơn có Prefix T thuộc cái bàn này sẽ bị xóa hoặc bị cancel 
                                //Khi Thanh toán sẽ k có sales type là Table mà nó chỉ là 1 đơn bán bình thường nên nó sẽ k có + 'T' 
                                ...
                                if (!string.IsNullOrEmpty(model.RefTransId))
                                {
                                    ...
                                    string statusFieldX = _saleHeaderRepository.GetScalar(checkOrderStatusX, null, commandType: CommandType.Text);
                                ...
                                parameters.Add("PrefixSO", PrefixSO);
                                ...
                                {
                                    if ((model.DataSource == "POS" && model.Status.ToLower() != "i" && model.Status.ToLower() != "h" && model.Status.ToLower() != "hold") || (model.DataSource != "POS" && (model.IsCanceled == "C" || model.Status == "C")))
                                    {
                                        model.Status = "C";
                                        parameters.Add("CollectedStatus", "Completed");
                                    }
                                    else if (model.Status.ToLower() == "i")
                                    {
                                        parameters.Add("CollectedStatus", "InProcess");
                                    }
                                    else
                                    {
                                        parameters.Add("CollectedStatus", "Hold");
                                ...
                                foreach (var line in model.Lines)
                                {
                                    ...
                                    if (line.Quantity.HasValue && Math.Abs(line.Quantity.Value) > 0)
                                    {
                                        ...
                                        line.TaxAmt = (line.LineTotal * (line.TaxRate.HasValue ? line.TaxRate : 1)) / 100;
                                        //TSalesPayment payoo = new TSalesPayment();
                                        if (model.Payments != null && model.Payments.Count > 0)
                                        {
                                            var payoo = model.Payments.Where(x => x.PaymentCode == "PAYOO" && x.CustomF2 == "E").FirstOrDefault();

                                            var serial = model.SerialLines.Where(x => x.SerialNum == line.SerialNum).FirstOrDefault();

                                            if (model.DataSource == "POS" && (payoo == null || serial == null) && model.IsCanceled != "C" && (line.ItemType == "Member" || line.ItemType == "Class"))
                                            {
                                                TSalesLineSerialViewModel newSerialLine = new TSalesLineSerialViewModel();
                                                newSerialLine.ItemCode = line.ItemCode;
                                                newSerialLine.UomCode = line.UomCode;
                                            ...
                                            if (MWIEpay != null)
                                            {
                                                ...
                                                if (model.SalesMode == "SALES" && (line.ItemType.ToLower() == "pn" || line.ItemType.ToLower() == "pin"))
                                                {
                                                    ...
                                                    for (int iP = 1; iP <= line.Quantity; iP++)
                                                    {
                                                        ...
                                                        //var jsonString = acttiveVoucher.ToJson();
                                                        //lineLog.JsonBody = jsonString;
                                                        flag = "Begin Epay X " + "C2 " + MWIEpay.CustomField2 ?? "";
                                                        ...
                                                        if (resultEpay != null && resultEpay.Success)
                                                        {
                                                            ...
                                                            if (!string.IsNullOrEmpty(data.pinExpiryDate))
                                                            {
                                                                ...
                                                                //var date = DateTime.Now.ToString("yyyy").Substring(0, 2) + data.pinExpiryDate.Substring(4, 2) + "/" + data.pinExpiryDate.Substring(2, 2) + "/" + data.pinExpiryDate.Substring(0, 2);
                                                                var date = DateTime.Now.ToString("yyyy").Substring(0, 2) + data.pinExpiryDate.Substring(0, 2) + "/" + data.pinExpiryDate.Substring(2, 2) + "/" + data.pinExpiryDate.Substring(4, 2);
                                                                newSerialLine.ExpDate = DateTime.Parse(date);
                                                            }
                                                ...
                                                if (line.ItemType.ToLower() == "tp" || line.ItemType.ToLower() == "topup" || line.ItemType.ToLower() == "bp")
                                                {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        result.Success = false;
                                                        result.Code = 2000;
                                                        var message = $"Epay Message: URL(" + MWIEpay.CustomField1 + "), Terminal: " + MWIEpay.CustomField3 + ", Merchand: " + MWIEpay.CustomField2 + " Msg: " + resultEpay.Msg;
                                                        result.Message = message;
                                                        lineLog.EndTime = DateTime.Now;
                                                        lineLog.Remark = message;
                                                        lineLog.Status = "False";
                                        ...
                                        SOLines.Rows.Add(key, line.LineId, model.CompanyCode, line.ItemCode, line.SlocId, line.BarCode, line.UomCode, line.Quantity, line.Price, line.LineTotal,
                                            ...
                                            line.PriceListId, line.ProductId, line.WeightScaleBarcode, model.StoreId, line.BookletNo);
                                        ...
                                        if (line.Staffs != null && line.Staffs.Count > 0)
                                        {
                                            int sttStaff = 0;
                                            foreach (var staff in line.Staffs)
                                            {
                                                sttStaff++;
                                                staff.ItemLine = line.LineId;
                                                staff.LineId = sttStaff;
                                ...
                                parameters.Add("@Lines", SOLines.AsTableValuedParameter("[dbo].[SalesLineTableType]"));
                                ...
                                parameters.Add("@LineSerials", SOLineSerials.AsTableValuedParameter("[dbo].[SalesLineSerialTableType]"));
                                ...
                                if (model.VoucherApply != null && model.VoucherApply.Count > 0)
                                {
                                    ...
                                    foreach (var voucher in model.VoucherApply)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            orderLogModel.Type = "HoldVoucher";
                                            orderLogModel.Action = "Hold";
                                            ...
                                            string message = "";
                                            if (resultHold.Status != 1)
                                            {

                                                if (!string.IsNullOrEmpty(resultHold.Msg))
                                                {
                                                    message = resultHold.Msg;
                                                }
                                                if (!string.IsNullOrEmpty(resultHold.message))
                                                {
                                                    if (!string.IsNullOrEmpty(message))
                                                    {
                                                        message += " - " + resultHold.message;
                                                    }
                                                    else
                                                    {
                                                        message = resultHold.message;
                                                ...
                                                if (string.IsNullOrEmpty(message))
                                                {
                                                    message = "Can't get message from ";
                                                }
                                                newModel.Remarks = message;

                                            }
                                            if (string.IsNullOrEmpty(message))
                                            {
                                                newModel.Logs.Add(orderLogModel);
                                                await WriteLogRemoveBasket(newModel);
                                                newModel.Logs = null;
                                                holdList.Add(voucher.voucher_code);
                                                //message = "Test";
                                ...
                                if (model.Status.ToLower() != "h" && model.Status.ToLower() != "hold")
                                {
                                    ...
                                    if (luckyDraw != null && (luckyDraw.SettingValue == "true" || luckyDraw.SettingValue == "1"))
                                    {
                                        ...
                                        if (luckyModel != null && !string.IsNullOrEmpty(luckyModel.LuckyNo))
                                        {
                                            model.LuckyNo = luckyModel.LuckyNo;
                                            if (!string.IsNullOrEmpty(luckyModel.LoyaltyId))
                                            {
                                                model.LoyaltyId = luckyModel.LoyaltyId;
                                            }
                                ...
                                if (!string.IsNullOrEmpty(model.LoyaltyId))
                                {
                                    parameters.Add("LoyaltyId", model.LoyaltyId);
                                }
                                if (!string.IsNullOrEmpty(model.LuckyNo))
                                {
                                    parameters.Add("LuckyNo", model.LuckyNo);
                                ...
                                key = db.ExecuteScalar("USP_I_T_SalesOrder", parameters, commandType: CommandType.StoredProcedure, transaction: tran).ToString();
                                ...
                                if (memberCardValue != null && (memberCardValue.SettingValue == "true" || memberCardValue.SettingValue == "1"))
                                {
                                    List<string> lstCardNo = document.DocumentLines.Where(x => x.ItemType.ToLower() == "member" && !string.IsNullOrEmpty(x.SerialNum)).Select(x => x.SerialNum).ToList();
                                    if (lstCardNo.Count > 0)
                                    {
                                        string qryCheckCard = $"SELECT CardNo From S_CustomerCard WHERE CustomerId <> '{document.CardCode}' AND CardNo IN ('{string.Join("','", lstCardNo)}')";
                                        var cardNo = db.Query<string>(qryCheckCard, commandType: CommandType.Text, transaction: tran).ToList();
                                        if (cardNo != null && cardNo.Count > 0)
                                        {
                                            tran.Rollback();
                                            result.Success = false;
                                ...
                                if (model.Status == "I")
                                {
                                    //var payoo = SettingList.FirstOrDefault().GeneralSettings.Where(x => x.SettingId == "Payoo").FirstOrDefault();
                                    if (settingPayoo != null)
                                    {
                                        var MWIPayoo = settingPayoo.CustomField1;


                                        var PayooResult = PayooPaymentOrder(MWIPayoo, model.TotalPayable, "", key, model.CusName, model.CusAddress, "", "", model.TerminalId).Result;
                                        if (PayooResult.success == true)
                                        {
                                            RPFO.Utilities.Helpers.LogUtils.WriteLogData("C:\\RPFO.API.MWIEpayModel\\", "SalesOrders", "MWIPayooModel", PayooResult.ToJson());
                                            //result.Success = true;
                                            //result.Message = key;

                                            string keyCache = string.Format(PrefixCacheActionSO, model.StoreId, model.TerminalId);
                                            string storeCache = cacheService.GetCachedData<string>(keyCache);
                                            cacheService.ClearCacheData<string>(null, keyCache, timeQuickAction);
                                            model.TransId = key;
                                            //result.Data = model;
                                            //foreach (var line in model.Lines)
                                            //{
                                            //    model.SerialLines = new List<TSalesLineSerialViewModel>();
                                            //}
                                            var ConfirmData = ConfirmSOPayoo(model).Result;
                                            RPFO.Utilities.Helpers.LogUtils.WriteLogData("C:\\RPFO.API.MWIEpayModel\\", "SalesOrders", "ComfirmModel", ConfirmData.ToJson());
                                        ...
                                        {
                                            if (PayooResult.Msg.Contains("HttpClient.Timeout") || PayooResult.message == "Settimeout")
                                            {
                                                result.Success = true;
                                                result.Message = key;
                                                model.TransId = key;
                                            ...
                                            {
                                                result.Success = false;
                                                result.Message = PayooResult.message;
                                                model.TransId = key;
                                                result.Data = model;
                                                model.OrderId = Guid.NewGuid();
                                                var reject = RejectSO(model);
                                            }
                                            //result.Success = false;
                                            //result.Message = PayooResult.message;
                                            //model.TransId = key;
                                ...
                                //var openDraw = SettingList.FirstOrDefault().GeneralSettings.Where(x => x.SettingId == "WriteLog").FirstOrDefault(); 
                                //if (writeLog != null && (writeLog.SettingValue == "Database" || writeLog.SettingValue == "All"))
                                //{
                                //    await WriteLog(model);
                            ...
                            catch (Exception ex)
                            {
                                if (holdList != null && holdList.Count > 0)
                                {
                                    foreach (var voucher in holdList)
                                    {
                                        var resultUnhold = await UnholdVoucher(voucher, model.CusId, model.StoreId, key);
                                        SaleViewModel newModel = new SaleViewModel();
                                        newModel = model;
                                        newModel.Logs = new List<OrderLogModel>();
                                        OrderLogModel orderLogModel = new OrderLogModel();
                                        orderLogModel.CreatedBy = model.CreatedBy;
                                        orderLogModel.CompanyCode = model.CompanyCode;
                                        orderLogModel.Type = "UnholdVoucher";
                                        orderLogModel.Action = "Unhold";
                                        orderLogModel.Result = resultUnhold.Status.ToString();
                                        orderLogModel.Value = voucher;
                                        orderLogModel.CustomF1 = model.CusId;
                                        orderLogModel.CustomF2 = key;
                                        orderLogModel.CustomF3 = "";
                                        ...
                                        orderLogModel.Time = DateTime.Now;
                                        string message = "";
                                        if (resultUnhold.Status != 1)
                                        {

                                            if (!string.IsNullOrEmpty(resultUnhold.Msg))
                                            {
                                                message = resultUnhold.Msg;
                                            }
                                            if (!string.IsNullOrEmpty(resultUnhold.message))
                                            {
                                                if (!string.IsNullOrEmpty(message))
                                                {
                                                    message += " - " + resultUnhold.message;
                                                }
                                                else
                                                {
                                                    message = resultUnhold.message;
        ...
        public async Task WriteLog(SaleViewModel model)
        {
            ...
            if (logs != null && logs.Count > 0)
            {
                ...
                foreach (var line in logs)
                {
                    ...
                    //parameters.Add("CustomF4", line.CustomF4);
                    //parameters.Add("CustomF5", line.CustomF5);
                    //parameters.Add("CustomF6", line.CustomF6);
                    //parameters.Add("CustomF7", line.CustomF7);
                    //parameters.Add("CustomF8", line.CustomF8);
                    //parameters.Add("CustomF9", line.CustomF9);
                    //parameters.Add("CustomF10", line.CustomF10);
                    //parameters.Add("TerminalId", model.TerminalId);
                    //parameters.Add("CreatedBy", model.CreatedBy);
                ...
                _logRepository.Execute("USP_I_S_LogByTableType", parameters, commandType: CommandType.StoredProcedure);
        ...
        public void createHtml(string path)
        {
            ...
            //var path = Path.Combine(
            //           Directory.GetCurrentDirectory(),
            //           "wwwroot/Logs", "HTMLtoPDF.pdf");

            //var cssText = "";// File.ReadAllText(path);
            //var html = File.ReadAllText(path);

            //using (var memoryStream = new MemoryStream())
            //{
            //    var document = new iTextSharp.text.Document(iTextSharp.text.PageSize.A4, 50, 50, 60, 60);
        ...
        //public async Task<GenericResult> testPrint(string str, string filename, string width, string scale)
        //{
        //    GenericResult result = new GenericResult();
        //    try
        //    {
        ...
        //        var path = Path.Combine(
        //                   Directory.GetCurrentDirectory(),
        //                   "wwwroot/Logs", "Filename.pdf");


        //        //WriteHTML(path, "", filename, str);
        //        //var pathX = Path.Combine(
        //        //          Directory.GetCurrentDirectory(),
        //        //          "wwwroot/Logs", filename);
        //        //createHtml(pathX);
        ...
        //        //PdfMargin.Top = 0;
        //        //PdfMargin.Bottom = 0;
        //        //pdf.PrintDocument.DefaultPageSettings.Landscape = true;
        //        PaperSize paperSize = new PaperSize();
        //        paperSize.Width = int.Parse(width);//inch*100
        //        paperSize.Height = 1826 ;//inch*100
        //        paperSize.RawKind = (int)PaperKind.Custom;
        //        pdf.PrintSettings.PaperSize = paperSize;
        //        float scaleX = float.Parse(scale);
        //        pdf.PrintSettings.SelectSinglePageLayout(PdfSinglePageScalingMode.CustomScale, false, scaleX);

        //        //PaperSize psizee = new PaperSize();
        //        //psizee.Width = 10000;
        //        //psizee.Height = 3275;
        //        //psizee.RawKind = (int)PaperKind.Custom;
        //        //pdf.PrintSettings.PaperSize = psizee;
        //        //pdf.PrintSettings.Landscape = false;
        //        //pdf.PageScaling = 120;
        //        //pdf.PrintSettings.SelectSinglePageLayout(PdfSinglePageScalingMode.CustomScale, true, 120f);
        //        //pdf.PageSettings.SetMargins(0);

        //        //pdf.PrintSettings.PaperSize.RawKind = (int)PaperKind.Custom;
        //        //pdf.PrintSettings.PaperSize.Width = 80;// = new D.Printing.PaperSize("custom", 80, 3275);
        //        //pdf.PrintSettings.PaperSize.Height = 3275;

        //        //PdfDocument newPdf = new PdfDocument();
        //        //foreach (PdfPageBase page in pdf.Pages)
        //        //{
        //        //    //Create a new PDF page with Page size =A1 and PDFMargins at 0.
        //        //    PdfPageBase newPage = newPdf.Pages.Add(PdfPageSize.Note, new PdfMargins(0));
        ...
        //        //var PDF = Renderer.RenderUrlAsPdf("http://localhost:4200/shop");
        ...
        private void Document_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            e.Graphics.DrawString(HtmlString, new System.Drawing.Font("Arial", 10), Brushes.Black, 0F, 0F, new StringFormat());
        }

        public void WriteHTML(string baseDirectory, string eventName, string queueName, string text)
        {
            try
            {
                string fileName = $"{queueName}_{DateTime.Now.ToString("HHmmss")}_{DateTime.Now:fff}.html";
        ...
        public async Task WriteFileLog(SaleViewModel model, bool? modelJson)
        {
            ...
            if (logs != null && logs.Count > 0)
            {
                ...
                if (modelJson != null && modelJson == true)
                {
                    var modelClone = new SaleViewModel();
                    modelClone = model;
                    modelClone.Logs = null;
                    LogUtils.WriteLogData(path, "", filename + "Json", modelClone.ToJson());
                }
                else
                {
        ...
        public async Task<GenericResult> VoidEpay(string Url, List<EpayModel> list, string TransId)
        {
            GenericResult result = new GenericResult();
            try
            {
                ...
                foreach (var epay in list)
                {
                    ...
                    {
                        var resultEpay = await EpayVoidPINOrder(Url, epay.terminalID, epay.merchantID, epay.amount, epay.operatorID, epay.product, epay.transRef, TransId);
                    ...
                    {
                        //line.ItemCode
                        var resultEpay = await EpayVoidTopupOrder(Url, epay.terminalID, epay.merchantID, epay.amount, epay.operatorID, epay.product, epay.transRef, TransId);
                        //resultData.Add(resultEpay);
                        SThirdPartyLogLine lineLogVoid = new SThirdPartyLogLine();
                        //lineLogVoid.CompanyCode = model.CompanyCode;
                        //lineLogVoid.TransId = model.TransId;
                    ...
                    {
                        var resultEpay = await EpayVoidPaymentOrder(Url, epay.terminalID, epay.merchantID, epay.amount, epay.operatorID, epay.product, epay.transRef, TransId);
                        //resultData.Add(resultEpay);
                        SThirdPartyLogLine lineLogVoid = new SThirdPartyLogLine();
                        //lineLogVoid.CompanyCode = model.CompanyCode;
                        //lineLogVoid.TransId = model.TransId;
        ...
        public async Task<OMSResponseModel> ReedemVoucher(string voucher, string customerid, string storeCode, string transactionId)
        {
            ...
            {
                HttpClient client = GetHttpClient("");
                client.DefaultRequestHeaders.Add("X-ABEO-SECRET", MWIAuthCustom);
                string requestUri = "JAMwi/ReedemVoucher?customerid=" + customerid + "&voucherid=" + voucher + "&storeCode=" + storeCode + "&transactionId=" + transactionId;
                var model = new { customerid = customerid, voucherid = voucher, storeCode = storeCode, transactionId = transactionId };
                var content = new StringContent(model.ToJson(), Encoding.UTF8, "application/json");
                var response = await client.PostAsync(requestUri, content);
        ...
        public async Task<OMSResponseModel> ServayUpdateVoucher(string Url, S4VoucherDetail model)
        {
            //Pls, Not using voucher customer , If run function ServayUpdateVoucher
            //"/api/S4MWI/UpdateVoucher". It will update voucher used S4MWI and It will is Redeem Voucher.
        ...
        public async Task<OMSResponseModel> ServayUpdateVouchers(string Url, List<S4VoucherDetail> models)
        {
            //Pls, Not using voucher customer , If run thought function 
            //"/api/S4MWI/UpdateVoucher". it will update voucher used in S4MWI and It will is reddeemed voucher.
        ...
        public async Task<OMSResponseModel> ServaySarawakRefund(string Url, string merchantId, string merOrderNo, string orderNo, string notifyURL, decimal? refundAmt)
        {
            OMSResponseModel result = new OMSResponseModel();
            try
            {
                ...
                //string MWIAuthPassword = "1234";
                //string MWIAuthCustom = "";
                var modelAuth = new { userName = MWIAuthUser, password = MWIAuthPassword };
        ...
        public async Task<GenericResult> EpayPINOrder(string Url, string terminalID, string merchantID, decimal? amount, string operatorID, string product, string transId)
        {
            GenericResult result = new GenericResult();
            try
            {
                RPFO.Utilities.Helpers.LogUtils.WriteLogData("C:\\RPFO.API.MWIEpayModel\\", "SalesOrders", "Url", Url);
                HttpClient client = GetHttpClient(Url);
                ///api/Auth/Login
                string requestUriAuth = "/api/Auth/Login";
                ...
                var rsModelAuth = JsonConvert.DeserializeObject<OMSResponseModel>(responseStringAuth);
                //var rsModel = JsonConvert.DeserializeObject<JAMwiResponseModel>(responseString);
                RPFO.Utilities.Helpers.LogUtils.WriteLogData("C:\\RPFO.API.MWIEpayModel\\", "SalesOrders", "rsModelAuth", rsModelAuth.ToJson());
                ...
                var jsonFile = model.ToJson();
                RPFO.Utilities.Helpers.LogUtils.WriteLogData("C:\\RPFO.API.MWIEpayModel\\", "SalesOrders", "model POST", model.ToJson());
                var content = new StringContent(model.ToJson(), Encoding.UTF8, "application/json");
                var response = await client.PostAsync(requestUri, content);
                if (response != null)
                {
                    ...
                    {
                        //result.success = false;
                        //result.Msg = "Content response is null value";
                        //result.message = "Content response is null value";
                        //result.Status = -1;
                ...
                {
                    //result.success = false;
                    //result.Msg = "Response is null value";
                    //result.message = "Response is null value";
                    //result.Status = -1;

                    result.Success = false;
                    result.Message = "Response is null value";
                    //result.Status = -1;
                    //RPFO.Utilities.Helpers.LogUtils.WriteLogData("C:\\RPFO.API.MWIEpayModel\\", "SalesOrders", "Exception", ex.ToJson());
        ...
        public async Task<GenericResult> EpayVoidPINOrder(string Url, string terminalID, string merchantID, decimal? amount, string operatorID, string product, string transRef, string transId)
        {
            GenericResult result = new GenericResult();
            try
            {
        ...
        public async Task<OMSResponseModel> EpayTopupOrder(string Url, string terminalID, string merchantID, decimal? amount, string operatorID, string product, string accountNo, string transId)
        {
            OMSResponseModel result = new OMSResponseModel();
            try
            {
        ...
        public async Task<GenericResult> EpayVoidTopupOrder(string Url, string terminalID, string merchantID, decimal? amount, string operatorID, string product, string transRef, string transId)
        {
            GenericResult result = new GenericResult();
            try
            {
        ...
        public async Task<GenericResult> EpayPaymentOrder(string Url, string terminalID, string merchantID, decimal? amount, string operatorID, string product, string accountNo, string transId)
        {
            GenericResult result = new GenericResult();
            try
            {
        ...
        public async Task<GenericResult> EpayVoidPaymentOrder(string Url, string terminalID, string merchantID, decimal? amount, string operatorID, string product, string transRef, string transId)
        {
            GenericResult result = new GenericResult();
            try
            {
        ...
        public async Task<GenericResult> EpayRefundPaymentOrder(string Url, string terminalID, string merchantID, decimal? amount, string operatorID, string product, string transRef, string transId)
        {
            GenericResult result = new GenericResult();
            try
            {
        ...
        public async Task<OMSResponseModel> GrabAcceptRejectOrder(string Url, string TransId, string Status)
        {
            OMSResponseModel result = new OMSResponseModel();
            try
            {
                        ...
                        using (var tran = db.BeginTransaction())
                        {
                            try
                            {
                                ...
                                foreach (var line in model.Lines)
                                {
                                    ...
                                    //string getCus8 = $"select CustomField8 from M_Item  with (nolock)  where  ItemCode =N'{line.ItemCode}' ";
                                    //var Cus8Value = _saleHeaderRepository.GetScalar(getCus8, null, commandType: CommandType.Text);
                                    if (checkCompValue == "1")
                                    {
                                ...
                                {
                                    db.Execute($"Update T_SalesHeader set TerminalId = N'{model.TerminalId}', ModifiedBy = N'{model.ModifiedBy}', ModifiedOn = N'{model.ModifiedOn}', ShiftId = N'{model.ShiftId}', Status='O' , CreatedOn = '{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss:fff")}',  CollectedStatus = 'Open' where TransId=N'{model.TransId}' and CompanyCode= N'{model.CompanyCode}'", null, commandType: CommandType.Text, transaction: tran);
                                }
                                else
                                {
                                    db.Execute($"Update T_SalesHeader set TerminalId = N'{model.TerminalId}', ModifiedBy = N'{model.ModifiedBy}', ModifiedOn = N'{model.ModifiedOn}', ShiftId = N'{model.ShiftId}', Status='C' , CollectedStatus = 'Closed' where TransId=N'{model.TransId}' and CompanyCode= N'{model.CompanyCode}'", null, commandType: CommandType.Text, transaction: tran);
                                ...
                                if (model.DataSource != "POS")
                                {
                                    ...
                                    if (lineTotal == model.TotalPayable && hasMemberClass)
                                    {
                                        if (model.Payments != null && model.Payments.Count > 0)
                                        {
                                            db.Execute($"Update T_SalesHeader set TerminalId = N'{model.TerminalId}',  ModifiedBy = N'{model.ModifiedBy}', ModifiedOn = N'{model.ModifiedOn}', ShiftId = N'{model.ShiftId}', Status='C' ,  CreatedOn = '{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss:fff")}', CollectedStatus = 'Closed' where TransId=N'{model.TransId}' and CompanyCode= N'{model.CompanyCode}'", null, commandType: CommandType.Text, transaction: tran);
                                ...
                                if (setting != null && setting.SettingValue == "Grab")
                                {
                                    ...
                                    var resultCall = await GrabAcceptRejectOrder(MWIGrab.CustomField1, model.OMSId, "Accepted");
                    ...
                    try
                    {
                        ...
                        using (var tran = db.BeginTransaction())
                        {
                            try
                            {
                                ...
                                if (setting != null && setting.SettingValue == "Grab")
                                {
                                    if (MWIGrab != null && !string.IsNullOrEmpty(MWIGrab.CustomField1))
                                    {
                                        var resultCall = await GrabAcceptRejectOrder(MWIGrab.CustomField1, model.OMSId, "Rejected");
                                        if (resultCall != null && !resultCall.success.Value)
                                        {
                                            var data = JsonConvert.DeserializeObject<GrabResultModel>(resultCall.Data.ToString());
                                            //var data = listData.FirstOrDefault();
                                    ...
                                    else
                                    {
                                        tran.Rollback();
                                        result.Success = false;
                                        result.Message = "Please check Grab setup";
                                        return result;
                                    }
                                ...
                                {
                                    //,ModifiedBy = N'{model.ModifiedBy}' ,ModifiedOn = N'{DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss")}'
                                    db.Execute($"Update T_SalesHeader set Status='R', CollectedStatus = 'Rejected' , Reason = N'{model.Reason}',OrderId = N'{model.OrderId}', ModifiedBy = N'{model.ModifiedBy}' ,ModifiedOn = N'{DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss")}'  where TransId=N'{model.TransId}' and CompanyCode= N'{model.CompanyCode}' ", null, commandType: CommandType.Text, transaction: tran);

                                }
                                else
                                {
                                    db.Execute($"Update T_SalesHeader set Status='R', CollectedStatus = 'Rejected' , Reason = N'{model.Reason}', ModifiedBy = N'{model.ModifiedBy}',ModifiedOn = N'{DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss")}' where TransId=N'{model.TransId}' and CompanyCode= N'{model.CompanyCode}' ", null, commandType: CommandType.Text, transaction: tran);
        ...
        private string ColorRandom()
        {
            ...
            string result = ColorTranslator.ToHtml(randomColor);
            return result;
        }
                        ...
                        using (var tran = db.BeginTransaction())
                        {
                            try
                            {
                                ...
                                if (OMSSO != null && OMSSO.Count > 0)
                                {
                                    ...
                                    var settingData = await _settingService.GetGeneralSettingByStore(firstSO.CompanyCode, firstSO.StoreId);
                                    List<GeneralSettingStore> SettingList = new List<GeneralSettingStore>();
                                    if (settingData.Success)
                                    {
                                    ...
                                    // 2022-12-28 mở ra cho JA... check khi đóng đơn Event nếu chưa checkout thì không được đóng đơn
                                    if (Status.ToLower() == "closed")
                                    {
                                        ...
                                        if (setting1 != null && (setting1.SettingValue == "1" || setting1.SettingValue.ToLower() == "true"))
                                        {
                                            var checkRs = await _invoiceService.CheckARExistedBySoId(firstSO.TransId, firstSO.CompanyCode, firstSO.StoreId);
                                            if (checkRs.Success == false)
                                            {
                                                result.Success = false;
                                                result.Message = "Can't Closed Order. A/R of order : " + firstSO.TransId + " doesn't existed.";
                                                return result;
                                            }
                                        }
                                    }
                                    if (Status.ToLower() == "canceled" || Status.ToLower() == "cancelled" || Status.ToLower() == "failed")
                                    {
                                        if (firstSO.Status.ToLower() == "h" || firstSO.Status.ToLower() == "hold")
                                        {
                                            ...
                                            db.Execute($"Update T_SalesHeader set IsCanceled = 'Y',   CollectedStatus = 'Canceled' where TransId=N'{firstSO.TransId}' and CompanyCode=N'{firstSO.CompanyCode}' and StoreId=N'{firstSO.StoreId}'", null, commandType: CommandType.Text, transaction: tran);
                                        ...
                                        else
                                        {
                                            ...
                                            if (setting1 != null && (setting1.SettingValue == "1" || setting1.SettingValue.ToLower() == "true"))
                                            {
                                                var checkRs = _invoiceService.CheckARExistedBySoId(firstSO.TransId, firstSO.CompanyCode, firstSO.StoreId).Result;
                                                if (checkRs.Success)
                                                {
                                                    result.Success = false;
                                                    result.Message = "Can't Cancel Order. A/R of order : " + firstSO.TransId + " has existed.";
                                                    return result;
                                                }
                                            }
                                            // tạm thời bỏ lại vào Elese để k tạo đơn đảo khi cancel
                                            else
                                            {
                                        ...
                                        if (model.IsCanceled == "C" && payment.CustomF2 == "E")
                                        {
                                            ...
                                            if (payment.CustomF3 == "EWallet")
                                            {
                                                ...
                                                if (MWIEpay != null)
                                                {
                                                    ...
                                                    var CancelResult = await EpayVoidPaymentOrder(MWIEpay.CustomField1, MWIEpay.CustomField3, MWIEpay.CustomField2, value, model.StoreId, "", payment.RefNumber, model.OrderId.ToString() + "-" + stt.ToString());
                                                    if (CancelResult.Success)
                                                    {
                                                        //var resultData = CancelResult.Data as EpayModel;
                                                        payment.CustomF1 = CancelResult.Message.ToString();
                                                ...
                                                else
                                                {
                                                    tran.Rollback();
                                                    result.Success = false;
                                                    result.Message = "Can't get Epay. Please check Setup";
                                                    return result;
                                                }
        ...
        public async Task<GenericResult> Delete(string CompanyCode, string StoreId, string TransId)
        {
            GenericResult result = new GenericResult();
            ...
            using (IDbConnection db = _saleHeaderRepository.GetConnection())
            {
                try
                {
                    ...
                    ////AssignStaff
                    //string queryCheckAndCreate = "";
                    //queryCheckAndCreate = $"delete T_SalesHeader where TransId = N'{TransId}' and CompanyCode = N'{CompanyCode}' and StoreId = N'{StoreId}'";
                    //db.Execute(queryCheckAndCreate);
                    //db.Close();
                    //result.Success = true;
                    var parameters = new DynamicParameters();
                    ...
                    var data = await _saleHeaderRepository.GetAllAsync($"USP_D_T_SalesOrder", parameters, commandType: CommandType.StoredProcedure);
                    db.Close();
                    result.Success = true;
        ...
        public async Task<GenericResult> CancelByTransID(string TransId)
        {
            GenericResult result = new GenericResult();
            ...
            using (IDbConnection db = _saleHeaderRepository.GetConnection())
            {
                try
                {
                    ...
                    //AssignStaff
                    string queryCheckAndCreate = "";
                    queryCheckAndCreate = $"update  T_SalesHeader set IsCanceled = N'Y'  where TransId = N'{TransId}' ";
                    db.Execute(queryCheckAndCreate);
                    db.Close();
                    result.Success = true;
        ...
        public async Task<GenericResult> GetCheckoutOpentList(string companycode, string storeId, string Type, string fromdate, string todate, string ViewBy)
        {
            ...
            try
            {
                ...
                query += $" and isnull(DataSource,'') <> 'POS' and Status ='O' and isnull(IsCanceled,'') ='N'  and PosType ='E' order by CreatedOn desc";
        ...
        public async Task<GenericResult> GetOrderById(string Id, string CompanyCode, string StoreId, bool? BreakByGroup = false)
        {
            GenericResult result = new GenericResult();
            string Flag = "";
            ...
            try
            {
                ...
                string queryLineSerial = $"USP_S_T_SalesLineSerial  N'{CompanyCode}' ,N'{Id}', N'{StoreId}'";
                //$"select t1.* , t2.ItemName , t3.UOMName from T_SalesLineSerial t1 with(nolock) left join M_Item t2 with(nolock)  on t1.ItemCode = t2.ItemCode AND T1.CompanyCode = t2.CompanyCode left join M_UOM t3 with(nolock)  on t1.UOMCode = t3.UOMCode where t1.TransId = N'{Id}' and t1.CompanyCode = N'{CompanyCode}'";
                ...
                using (IDbConnection db = _saleHeaderRepository.GetConnection())
                {
                    try
                    {
                        ...
                        order.Payments = payments;
                        order.Customer = customer;
                        var printVoucherHeader = SettingList.FirstOrDefault().GeneralSettings.Where(x => x.SettingId == "PrintVoucherHeader").FirstOrDefault();
                        if (printVoucherHeader != null && (printVoucherHeader.SettingValue == "true" || printVoucherHeader.SettingValue == "1"))
                        {
                            if (order.Lines != null && order.Lines.Count > 0)
                            {
                                var checkVoucher = order.Lines.Where(x => x.IsVoucher != null && x.IsVoucher == true).ToList();
                        ...
                        if (BreakByGroup != null && BreakByGroup.Value == true)
                        {
                            //List<string> GroupLines = new List<string>();
                            var GroupLines = order.Lines.Select(p => p.ItemTypeS4).Distinct();
                            foreach (string group in GroupLines)
                            {
                                var orderByGroup = new SaleViewModel();
                                orderByGroup = order.Clone();
                                var orderBreankLines = order.Lines.Where(x => x.ItemTypeS4 == group);
                                orderByGroup.Lines = orderBreankLines.ToList();
                                orderByGroup.TotalAmount = orderByGroup.Lines.Sum(x => (x.Quantity * x.Price));
                                order.BreakOrders.Add(orderByGroup);
                            }
                            //GroupLines = order.Lines.Distinct(x => x.Item).ToList();
                            //order
                            //var orderBreankLines = order.Lines.where(x=>x)
                        }
            ...
            {
                result.Success = false;
                result.Message = Flag + " _ " + ex.Message;
            }
            return result;
        }
            ...
            try
            {
                ...
                using (IDbConnection db = _saleHeaderRepository.GetConnection())
                {
                    try
                    {
                        ...
                        order.Customer = customer;
                        order.Deliveries = deliveryLines.ToList();
                        if (!string.IsNullOrEmpty(order.POSType) && order.POSType == "E")
                        {
                            if (!string.IsNullOrEmpty(order.ContractNo))
                            {
                                string queryCheck = $"select  stuff((SELECT distinct ', ' + cast(TransId as nvarchar(1000)) from T_InvoiceHeader with (nolock) where InvoiceType = 'CheckOut' and ContractNo = N'{order.ContractNo}' and isnull(IsCanceled, 'N') = 'N'  FOR XML PATH('')),1,1,'') transId  ";
                                var stringTran = _customerRepository.GetScalar(queryCheck, null, commandType: CommandType.Text);
                                if (!string.IsNullOrEmpty(stringTran))
                                {
                                    order.CheckOutOn = stringTran;
                                }
        ...
        public async Task<GenericResult> CloseOMSEvent(CloseEventViewModel model)
        {
            ...
            using (IDbConnection db = _saleHeaderRepository.GetConnection())
            {
                ...
                try
                {
                    ...
                    string checkQuery = $"select count(*) from T_InvoiceHeader with(nolock) where CompanyCode = N'{model.CompanyCode}' AND ContractNo = N'{model.ContractNo}'";
                    string resultValue = db.ExecuteScalar(checkQuery, null, commandType: CommandType.Text).ToString();
                    if (resultValue == "" || resultValue == "0")
                    {
                        //tran.Commit();

                        result.Success = false;
                        result.Message = "Can't Closed Order. A/R of contract : " + model.ContractNo + " doesn't existed.";
                    }
                    else
                    {
                        ...
                        db.Execute(upSql, null, commandType: CommandType.Text);
                        result.Success = true;
                    }
                    db.Close();
                }
        ...
        public async Task<GenericResult> PrintReceiptAsync(string companyCode, string transId, string storeId, string printStatus,
            string size, string printName, bool? BreakByGroup = false)
        {
            GenericResult result = new GenericResult();
            //result.Success = true;
            //bool BreakByGroup = false;
            try
            {
                SaleViewModel model = null;

                GenericResult SOData = await GetOrderById(transId, companyCode, storeId, BreakByGroup);
                ...
                if (model != null)
                {
                    ...
                    if (BreakByGroup != null && BreakByGroup == true)
                    {
                        //foreach
                        var peri = _peripherallService.GetAll(model.CompanyCode, model.StoreId, model.TerminalId, "").Result;
                        if (peri != null && peri.Success)
                        {
                            var data = peri.Data as List<MTerminalPeripherals>;
                            if (data != null && data.Count() > 0)
                            {

                                foreach (var order in model.BreakOrders)
                                {
                                    string printNameBreak = "";
                                    var checkDta = data.Where(x => x.Type == "Printer" && x.Condition1 == "ItemGroup" && x.Condition2 == order.Lines.FirstOrDefault().ItemTypeS4);
                                    if (checkDta != null && checkDta.Count() > 0)
                                    {
                                        if (string.IsNullOrEmpty(checkDta.FirstOrDefault().Value))
                                        {
                                            checkDta.FirstOrDefault().Value = "";
                                        }
                                        printNameBreak = checkDta.FirstOrDefault().Value;
                                        RPFO.Application.PrintLayout.PrintReceiptByGroup receipt = new PrintLayout.PrintReceiptByGroup();
                                        receipt.SetModel(order, printStatus);
                                        receipt.CreateDocument();
                                        receipt.Print(printNameBreak);
                    ...
                    {
                        if (model.LuckyNo != null)
                        {
                            // in form Print Lucky -- Print Lucky 55
                            if (size == "57")
                            {
                                RPFO.Application.PrintLayout.PrintLucky_55 receipt = new PrintLayout.PrintLucky_55();
                            ...
                            {
                                RPFO.Application.PrintLayout.PrintLucky receipt = new PrintLayout.PrintLucky();
                                receipt.SetModel(model, printStatus);
                                receipt.CreateDocument();
                                receipt.Print();
                            }
                        ...
                        {
                            // in form Print Receipt -- Print Receipt 55
                            if (size == "57")
                            {
        ...
        public async Task<OMSResponseModel> PayooPaymentOrder(string Url, decimal? amount, string accountNo, string transId, string CustomerName,
            string CustomerAddress, string CustomerPhone, string CustomerEmail, string terminalID)
        {
            OMSResponseModel result = new OMSResponseModel();
            try
            {
                ...
                string requestUri = "Payoo/CreateOrder";
                var date = DateTime.Now;
                date = date.AddMinutes(5);
                var orderExpiredDate = date.ToString("yyyyMMddHHmmss");
                if (CustomerName.Length > 32)
                {
                    CustomerName = CustomerName.Substring(0, 32);
                }
                if (terminalID.Length > 32)
                {
                    terminalID = terminalID.Substring(0, 32);
                }
                var model = new
                {
                    orderCode = transId,
                    ...
                    orderAmount = amount,
                    orderLinkNotify = Url + "Payoo/Notify",
                    accountName = "vietwash",//payoo cấp
                                             //staffCode = string.Empty,
                    customerName = CustomerName,
                    customerAddress = CustomerAddress,
                    customerPhone = CustomerPhone,
                    customerEmail = CustomerEmail,
                    orderNote = "Thanh toan don hang",
                    orderDetail = "plus",
                    terminalID = terminalID,
                    createShopCode = "vietwash1", //payoo cấp,
                    createDate = "",
                    status = 0
        ...
        public async Task<GenericResult> ConfirmSOPayoo(SaleViewModel model)
        {
            model.OrderId = Guid.NewGuid();
            //GenericResult result = new GenericResult();
            var result = CreateSaleOrderByTableType(model).Result;
            return result;
        }
    }
}
